...
def main_window():
    ...
    def ok_cmd():
        ...
        """
        from health_db_clientv2 import upload_patient_data_to_server
    ...
    def image_cmd():
        ...
        if filename == "":  # cancel button error fix
            root.after(30000, image_cmd)
            return
    ...
    # Image
    pil_image_raw = Image.open("pillow.jpg")
    pil_image = pil_image_raw.resize((200, 200))
    tk_image = ImageTk.PhotoImage(pil_image)
    image_label = ttk.Label(root, image=tk_image)
    image_label.image = tk_image  # creates new field in class, allows change of image
    ...
    # Start GUI
    root.after(30000, image_cmd)  # after 2000 ms, run the image command. Only works once, must put in command ^
    root.mainloop()
